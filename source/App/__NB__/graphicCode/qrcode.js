// 参考：https://555chy.github.io/qrcodejs/qrcode.js
...
for (function (a) {
    QRCodeGenerate= function (options) {


        var new_qrcode=new QRCode(options.typeNumber || -1, options.correctLevel || QRErrorCorrectLevel.H);

        new_qrcode.addData(options.text);
        new_qrcode.make();

        var moduleCount= new_qrcode.getModuleCount();

        var qr_data=[];

        for(var h=0;h<moduleCount;h++){
            var data_row=[];
            for (i = 0; i < moduleCount; i++) {

                data_row.push(new_qrcode.isDark(h, i));

            }
            qr_data.push(data_row);
        }
